...
- `ImageTrimmer` now crops via ChunkyPNG's `crop`/`border!` instead of a manual pixel-copy loop.
- `PreviewChecker#deep_render_check`/`#basic_check` share `passed_result`/`failed_result` helpers instead of duplicating `CheckResult.new` construction.
- Removed the unused `Configuration.config`/`Configuration.configure` class methods (module-level `LookbookVisualTester.config` is the only one in use).
- `lib/lookbook_visual_tester.rb` now requires `update_previews` directly, instead of relying on load order from specs.
